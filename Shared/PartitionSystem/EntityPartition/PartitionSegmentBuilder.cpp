...
PartitionSegmentBuilder &PartitionSegmentBuilder::From(vec2<unsigned int> const &from) {
    _startValue = from;
    return *this;
}

PartitionSegmentBuilder &PartitionSegmentBuilder::To(vec2<unsigned int> const &to) {
    _endValue = to;
    return *this;
}
...
PartitionSegment PartitionSegmentBuilder::Build(Timer *timer) {
    ...
    return PartitionSegment(Tween<vec2<unsigned int>>(timer, _startValue, _start, _endValue, TimeRef(_start.getMilliseconds() + _duration), _curvingOption));
}
...
const vec2<unsigned int> &PartitionSegmentBuilder::getStartValue() const {
    return _startValue;
}

const vec2<unsigned int> &PartitionSegmentBuilder::getEndValue() const {
    return _endValue;
}
