...
public :
    void BroadcastEntities(uint16_t, const std::shared_ptr<RType::EventManager> &eventManager, std::vector<std::shared_ptr<PlayerRef>> &);
    void BroadcastEntitiesThatStillExist(uint16_t, const std::shared_ptr<RType::EventManager> &eventManager, const std::vector<std::shared_ptr<PlayerRef>> &);
};
