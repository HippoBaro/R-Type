...
MenuCreateRoom::MenuCreateRoom(std::shared_ptr<RType::EventManager> &eventManager) : _eventListener(eventManager.get()) {
...
void MenuCreateRoom::specialDrawing(sf::RenderTexture &context, sf::Text &text) {
    _frame++;
    if (_frame > 30) {
...
const std::string MenuCreateRoom::getChannelName() {
